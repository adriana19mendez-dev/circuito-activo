...
def button(lang, page, label, secondary=False):
    ...
    return f'<a class="{cls}" href="{link(lang,page)}">{escape(label)}<span class="icon-arrow" aria-hidden="true"></span></a>'
...
def home(lang):
    ...
    group = '<div class="ticker-group">' + ''.join(f'<span>{w}<i class="ticker-separator" aria-hidden="true"></i></span>' for w in words) + '</div>'
